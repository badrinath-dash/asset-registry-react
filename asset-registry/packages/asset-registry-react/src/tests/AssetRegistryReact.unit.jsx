...
import AssetRegistryReact from '../AssetRegistryReact';

// This sets up the enzyme adapter
const adapter = new EnzymeAdapterReact16();
Enzyme.configure({ adapter });

describe('AssetRegistryReact', () => {
    it('renders with default name', () => {
        const wrapper = mount(<AssetRegistryReact />);
        assert.include(wrapper.text(), 'Hello, User!');
        wrapper.unmount();
    });

    it('renders with custom name', () => {
        const wrapper = mount(<AssetRegistryReact name="World" />);
